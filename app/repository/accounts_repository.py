...
class AccountRepository:
    ...
    def get_accounts_by_user_id(user_id: int):
        with engine.connect() as conn:
            query = text("SELECT id, user_id, account_name, balance FROM accounts WHERE user_id = :user_id")
    ...
    def create_account(user_id: int, account_name: str, balance: float = 0.0):
        with engine.begin() as conn:
            ...
            result = conn.execute(query, {"user_id": user_id, "account_name": account_name, "balance": balance})
            row = result.fetchone()
    ...
    def delete_account(account_id: int, user_id: int):
        with engine.begin() as conn:
            query = text("DELETE FROM accounts WHERE id = :id AND user_id = :user_id RETURNING id")
